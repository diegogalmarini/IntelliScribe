...
export const Dialer: React.FC<DialerProps> = ({ user, onNavigate }) => {
    ...
    const [errorMessage, setErrorMessage] = useState(''); // Para mostrar errores de permisos al usuario
    ...
    const handleCall = async () => {
        ...
        // Formato internacional directo (Usuario ve + en pantalla, añadimos + al string)
        ...
        try {
            ...
            setStatus('Error');
            // Mostramos el error real (ej: "Microphone access denied")
    ...
    return (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 sm:w-80 w-full h-full bg-white dark:bg-card-dark sm:rounded-2xl shadow-2xl border border-slate-200 dark:border-border-dark overflow-hidden flex flex-col z-50 animate-in slide-in-from-bottom-5">
            ...
            {/* Error Message Toast */}
            {errorMessage && (
                <div className="absolute top-14 left-0 right-0 bg-red-500 text-white text-xs p-2 text-center z-10 animate-fade-in">
                    {errorMessage}
                </div>
            )}
