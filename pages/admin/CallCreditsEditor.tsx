...
import { CallCreditPack } from '../../types';
import { Plus, Trash2, Save, RefreshCw, AlertCircle, Check, ArrowUp, ArrowDown, Phone } from 'lucide-react';

export const CallCreditsEditor: React.FC = () => {
    const [packs, setPacks] = useState<CallCreditPack[]>([]);
    ...
    const loadData = async () => {
        setLoading(true);
        const data = await adminService.getCallCreditPacks();
        setPacks(data);
        setLoading(false);
    };

    const handlePackChange = (id: string, field: keyof CallCreditPack, value: any) => {
        setPacks(prev => prev.map(p => p.id === id ? { ...p, [field]: value } : p));
    };

    const savePack = async (pack: CallCreditPack) => {
        setSavingId(pack.id);
        setSuccessId(null);
        setError(null);

        const success = await adminService.updateCallCreditPack(pack.id, pack);
    ...
    const addPack = async () => {
        const newPack: Partial<CallCreditPack> = {
            name: 'Nuevo Pack de Llamadas',
            minutes: 60,
            price: 15,
            checkout_url: '',
            is_active: true,
        ...
        const success = await adminService.createCallCreditPack(newPack);
        if (success) {
            loadData();
        } else {
    ...
    const deletePack = async (id: string) => {
        ...
        const success = await adminService.deleteCallCreditPack(id);
        if (success) loadData();
    };
    ...
    const movePack = async (id: string, direction: 'up' | 'down') => {
        ...
        await Promise.all([
            adminService.updateCallCreditPack(newPacks[index].id, { order: newPacks[index].order }),
            adminService.updateCallCreditPack(newPacks[swapIndex].id, { order: newPacks[swapIndex].order })
        ]);
    };
    ...
    if (loading) return (
        <div className="flex items-center justify-center h-64 text-slate-400 gap-2">
            ...
            <span className="text-sm">Cargando packs de créditos de llamada...</span>
        </div>
    );
        ...
        <div className="pb-20 space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Créditos de Llamada (Packs)</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-sm">Gestiona los paquetes de créditos para llamadas VoIP internacionales.</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6">
                    ...
                    <div key={pack.id} className={`bg-white dark:bg-[#0A0D13] rounded-xl border p-6 space-y-6 transition-all shadow-sm hover:shadow-md border-slate-200 dark:border-[#1f1f1f]`}>
                        <div className="flex flex-wrap items-start justify-between gap-4">
                            <div className="flex-1 min-w-[280px]">
                                ...
                                <input
                                    ...
                                    placeholder="Nombre del Pack (ej: Pack Premium)"
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 dark:bg-white/5 p-5 rounded-xl border border-slate-100 dark:border-white/5">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">Créditos / Minutos</label>
                ...
                {packs.length === 0 && (
                    <div className="text-center py-20 bg-slate-50 dark:bg-white/5 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10">
                        ...
                        <h3 className="text-lg font-bold text-slate-600 dark:text-slate-400">No hay packs de créditos configurados</h3>
                        <p className="text-slate-400 dark:text-slate-500 text-sm mb-6">Empieza creando tu primer paquete de créditos de llamada.</p>
                        <button
                            onClick={addPack}
